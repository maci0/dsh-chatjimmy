 ...
 *
 * @module dsh-chatjimmy/tests/client
 */
...
function createClient(options: { status?: string, writable?: boolean, acceptWrites?: boolean, value?: Record<string, unknown>, user?: Record<string, unknown> } = {}): any {
  ...
  const snapshot = {
    ...
    value: options.value ?? { baseUrl: 'https://chatjimmy.ai', model: 'llama3.1-8B', topK: 8, contextWindow: 6144, streamIdleTimeoutMs: 300_000 },
    base: {},
    user: options.user ?? {},
    revision: 1,
...
test('apply binds the namespace and registers the row card', () => {
  ...
  assert.equal(client.configNs, 'chatjimmy')
  assert.equal(client.namespace, 'chatjimmy')
  assert.equal(client.locale.ns, 'chatjimmy')
  ...
  assert.equal(client.slot.key, 'dsh-chatjimmy#chatjimmy')
  assert.equal(client.slot.locale, 'chatjimmy')
})
...
test('the summary view names the configured model and host', () => {
  const client = createClient()
  client.apply()
  assert.equal(client.render({ view: 'summary' }), 'summary:llama3.1-8B,https://chatjimmy.ai')
})
...
test('the page renders every editable field with its stored value', () => {
  ...
  assert.deepEqual(
    ...
    ['labelBaseUrl', 'labelModel', 'labelTopK', 'labelContextWindow', 'labelStreamIdleTimeoutMs'],
  )
  assert.deepEqual(
    client.inputs(tree).map((input: Element) => input.props.value),
    ['https://chatjimmy.ai', 'llama3.1-8B', '8', '6144', '300000'],
  )
...
test('a save writes only the fields that moved, as one mutation', async () => {
  ...
  setField(client, 'labelModel', 'llama3.1-70B')
  setField(client, 'labelTopK', '12')
  const tree = client.render({ view: 'page' })
  findAll(tree, 'button').find((button) => textOf(button) === 'save')?.props.onClick()
  await client.flush()

  assert.deepEqual(client.mutations, [[
    { op: 'set', path: ['model'], value: 'llama3.1-70B' },
    { op: 'set', path: ['topK'], value: 12 },
  ]])
})
...
test('a save the Host refuses is reported as refused', async () => {
  ...
  client.apply()
  const tree = setField(client, 'labelModel', 'llama3.1-70B')
  findAll(tree, 'button').find((button) => textOf(button) === 'save')?.props.onClick()
  await client.flush()
...
test('the card refuses what it can see before the write', async () => {
  const client = createClient()
  client.apply()

  let tree = setField(client, 'labelBaseUrl', 'chatjimmy.ai')
  findAll(tree, 'button').find((button) => textOf(button) === 'save')?.props.onClick()
  await client.flush()
  assert.match(textOf(client.render({ view: 'page' })), /baseUrlInvalid/)

  tree = setField(client, 'labelBaseUrl', 'https://chatjimmy.ai')
  tree = setField(client, 'labelModel', '   ')
  findAll(tree, 'button').find((button) => textOf(button) === 'save')?.props.onClick()
  await client.flush()
  assert.match(textOf(client.render({ view: 'page' })), /modelInvalid/)

  tree = setField(client, 'labelModel', 'llama3.1-8B')
  tree = setField(client, 'labelTopK', '0')
  findAll(tree, 'button').find((button) => textOf(button) === 'save')?.props.onClick()
  await client.flush()
...
test('an overridden row offers a reset that clears exactly the user layer', async () => {
  ...
  reset?.props.onClick()
  await client.flush()
  // The retry policy is patch-only: the card never clears a field it cannot edit.
